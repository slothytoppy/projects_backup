...
void _dir_close(void){
    fprintf(stderr, "directory couldnt close");
}
...
void remove_dir(char *name){
    ...
    printf("REMOVED:%s\n", name);
        ...
        perror(name);
        printf("\033[31;1;1mERROR FILE:%s name too long [MAX_FILE_NAME is usually 255 characters for linux]\033[0m\n", name);
        exit(1);
    }
    else if(errno==ERROR_ACCESS){
        perror(name);
        printf("ERROR DIR: insufficient permissions to remove dir %s\n", name);
	exit(1);
    }
}
